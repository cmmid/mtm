...
The "preferential attachment" model by Barabási and Albert, which is built by 
adding nodes one at a time, and each time a node is added, it is connected 
...
You can also modify properties of the vertices and of the edges, using `V()` 
and `E()` respectively.
...
showing the animated network the first `nanim` times, and returning a `data.table` with the results of each simulation.
